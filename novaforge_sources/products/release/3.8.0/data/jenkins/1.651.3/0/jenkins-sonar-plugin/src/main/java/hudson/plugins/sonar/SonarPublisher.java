...
public class SonarPublisher extends Notifier {
  ...
  private final String forgeProjectId;
  ...
  public SonarPublisher(String installationName,
    ...
    String mavenInstallationName, String rootPom, String jdk, SettingsProvider settings, GlobalSettingsProvider globalSettings, boolean usePrivateRepository, final String forgeProjectId) {
  ...
  public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws IOException, InterruptedException {
    ...
    {
      listener.getLogger().println("[Novaforge] : Add additional Sonar permissions");
      String forgeprojectid = forgeProjectId;
      if ((forgeprojectid == null) || forgeprojectid.trim().isEmpty())
      {
        forgeprojectid = build.getProject().getName().split("_")[0];
      }
      forgeprojectid = forgeprojectid.trim();
      listener.getLogger().println("[Novaforge] : forgeprojectid = " + forgeprojectid);
      final ModuleName moduleName = ((hudson.maven.MavenModuleSetBuild) build).getProject().getRootModule()
          .getModuleName();
      final String projectKee = moduleName.toString();
      
      // Retrieve credentials (Authentication token or username+password)
      String serveradminlogin = null;
      String serveradminpassword = null;
      if (StringUtils.isNotEmpty(sonarInstallation.getServerAuthenticationToken())) 
      {
        serveradminlogin = sonarInstallation.getServerAuthenticationToken();
      } 
      else 
      {
        serveradminlogin = "admin";
        if (null != sonarInstallation.getSonarLogin())
        {
          serveradminlogin = sonarInstallation.getSonarLogin();
        }
        serveradminpassword = "admin";
        if (null != sonarInstallation.getSonarPassword())
        {
          serveradminpassword = sonarInstallation.getSonarPassword();
        }
      }
      
      // Check if credentials are set
      if (StringUtils.isEmpty(serveradminlogin)) 
      {
        listener.getLogger().println("[Novaforge] : Sonar credentials not correctly set !!");
        listener.getLogger().println("[Novaforge] : Check Sonar credentials setting in Jenkins administration UI.");
        build.setResult(Result.FAILURE);
        sonarSuccess = false;
      }
      else 
      {
        // Perform permission addition HTTP request  
       	final WsClient wsClient = WsClientFactories.getDefault().newClient(HttpConnector.newBuilder().url(sonarInstallation.getServerUrl())
  				.credentials(serveradminlogin, serveradminpassword).build());
  
    		WsResponse response = wsClient.wsConnector().call(
    	    	      new PostRequest("api/membership/update_permissions")
    	    	        .setParam("project", projectKee)
    	    	        .setParam("forgeprojectid", forgeprojectid));                 
  
        if (!response.isSuccessful())
        {
          listener.getLogger().println("[Novaforge] : Sonar permissions update FAILED !!");
          listener.getLogger().println("[Novaforge] : Check Sonar credentials setting in Jenkins administration UI.");
          build.setResult(Result.FAILURE);
          sonarSuccess = false;
        }  
