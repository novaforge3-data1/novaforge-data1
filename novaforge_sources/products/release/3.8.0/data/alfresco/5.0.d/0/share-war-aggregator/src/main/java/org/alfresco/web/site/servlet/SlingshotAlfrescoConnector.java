 ...
 * <p>
 * The user name from the header is also used by {@link SSOAuthenticationFilter} for incoming request to Share
 * to return the user name from {@link HttpServletRequest#getRemoteUser}.
 * <p>
 * The name of the header to be used is defined in the userHeader element of the Alfresco Connector definition
 * (see share-config-custom.xml.sample). Also note the class element specifies this class. For example:
 * 
 * <pre>
 * &lt;connector&gt;
 ...
 * </pre>
 * 
 * The Alfresco global property {@code external.authentication.proxyHeader} still needs to be configured on
 * the Repository side to define which header will be used. For example:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * When using the default Alfresco Repository header (X-Alfresco-Remote-User") Share and the Alfresco
 * Repository must be protected against direct access from other clients. The same is true when using a
 * configurable header. The reason is that Share and Alfresco just accept the header value as valid. Without
 * this protection, it would be possible to log in as any user simply by setting the header.
 * 
 * @author adavis
 * @author kroast
 */
public class SlingshotAlfrescoConnector extends RequestCachingConnector
{
  ...
  public SlingshotAlfrescoConnector(final ConnectorDescriptor descriptor, final String endpoint)
  {
   ...
   */
  @Override
  public void setConnectorSession(final ConnectorSession connectorSession)
  {
  ...
  protected void applyRequestHeaders(final RemoteClient remoteClient, final ConnectorContext context)
  {
    ...
    final ConnectorSession connectorSession = getConnectorSession();
    if (connectorSession != null)
    {
      final Map<String, String> cookies = new HashMap<String, String>(8);
      for (final String cookieName : connectorSession.getCookieNames())
      {
    ...
    final Map<String, String> headers = new HashMap<String, String>(8);
    if (context != null)
    {
      headers.putAll(context.getHeaders());
    }

    // Proxy the authenticated user name if we have password-less credentials (indicates SSO auth over a
    // secure connection)
    if (getCredentials() != null)
    {
      final String userHeader = getUserHeader();
      if (userHeader != null)
      {
        // TODO: This is not ideal - for scenarios where the request has come through a Spring Dispatcher
        // servlet
        ...
        if (user != null)
        {
          ...
          if (!org.apache.commons.codec.binary.Base64.isBase64(user))
          {
            try
            {
              user = org.apache.commons.codec.binary.Base64.encodeBase64String((new String(user
                  .getBytes("ISO-8859-1"), "UTF-8")).getBytes("UTF-8"));
            }
        ...
        {
          user = (String) getCredentials().getProperty(Credentials.CREDENTIAL_USERNAME);
          final String pass = (String) getCredentials().getProperty(Credentials.CREDENTIAL_PASSWORD);
          if (pass == null)
          {
          ...
          {
            headers.put(userHeader, user);
            headers.put("Remote-User-Encode", Boolean.FALSE.toString());
          }
        }
      }
    }
